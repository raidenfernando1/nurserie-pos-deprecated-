...
export async function DELETE(
    ...
    try {
        ...
        if (result.length === 0) {
            return NextResponse.json({ success: false, error: "Cashier not found or not authorized" }, { status: 404 });
        }
        ...
        return NextResponse.json({
            ...
            deleteCashierId: result[0],
        },
            { status: 200 }
        );

    } catch (error) {
        console.log(">>> Error in DELETE /api/admin/cashier/[id]", error);
        return NextResponse.json(
            {
                success: false,
